#!/usr/bin/env python3
"""
linkedin-data-api - CoreClaw LinkedIn API Demo

This script demonstrates how to use CoreClaw's Web Data API for LinkedIn data extraction.
...
class LinkedinDataApi:
    """CoreClaw LinkedIn API client demo."""
    ...
    def search(self, query: str, limit: int = 100) -> List[CoreClawResult]:
        """Search LinkedIn data via CoreClaw API."""
...
def main():
    parser = argparse.ArgumentParser(description="linkedin-data-api - CoreClaw LinkedIn API Demo")
    ...
    client = LinkedinDataApi(api_key=args.api_key)
    client.search(args.query, args.limit)
    client.export(args.output, args.format)
    print(f"Done! Got {len(client.results)} records from CoreClaw LinkedIn API.")
